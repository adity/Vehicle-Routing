...
file_name = 'C:/Users/Aditya/Desktop/VRP Instances/P-n22-k2.vrp'
...
if __name__ == "__main__":
    ...
    # for rec in t4.out_db["y"]:
      # if rec.level > 0.00001:
        # print "y(" + rec.keys[0] + "): level=" + str(rec.level) + " marginal=" + str(rec.marginal)    
    #for rec in t4.out_db["z"]:
    #    print "z=" + str(rec.level)  
...
while (iter < 100):
    ...
    for q in qlist:
      for i in range(2,numnodes+1):
        if fiq0[i,q] < - theta[0]:  
          cgroutes[i,q] = reconstructpath(i,q)
          cgroutes[i,q].append(1) # append depot(1) at the end of the routes
          #print fiq0[i,q],i,q,cgroutes[i,q]
          
      
    #Add logic to discard routes with symmetry. 
    #############################################
    #################################################
    ...
    for i in cgroutes:
      routecost[K],ksitranspose[K] = get_routecst_ksitranspose(cgroutes[i],numnodes)
      routes[K] = cgroutes[i]
